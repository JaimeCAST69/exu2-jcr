pipeline {
    ...
    stages {
        ...
        stage('Parando los servicios...') {
            steps {
                bat '''
                    docker compose -p exu2-jcr down || exit /b 0
                '''
            }
        }
        ...
        stage('Eliminando imágenes anteriores...') {
            steps {
                bat '''
                    for /f "tokens=*" %%i in ('docker images --filter "label=com.docker.compose.project=exu2-jcr" -q') do (
                        docker rmi -f %%i
                    )
                    if errorlevel 1 (
        ...
        stage('Construyendo y desplegando servicios...') {
            steps {
                bat '''
                    docker network create --driver bridge exu2-jcr-net || exit /b 0
